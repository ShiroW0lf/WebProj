...
def generate_recommendations(data):
    ...
    # Get absolute correlations for each column with the target variable (assuming the last column as the target)
    correlations = corr_matrix.iloc[-1, :-1].abs().sort_values(ascending=False)
    ...
    if not correlations.empty:
        for column, correlation in correlations.items():
            if correlation >= 0.5:
                recommendations[column] = "scatter"
            elif correlation >= 0.3:
                recommendations[column] = "line"
            elif correlation >= 0.1:
                recommendations[column] = "bar"
...
if selected == 'Data Analysis':
    ...
    if 'data' in st.session_state:
        ...
        for column, visualization_type in recommendations.items():
            if visualization_type == "scatter":
                fig = px.scatter(data, x=column, y=data.columns[-1], title=f"{column.capitalize()} vs Target")
            elif visualization_type == "line":
                fig = px.line(data, x=column, y=data.columns[-1], title=f"{column.capitalize()} vs Target")
            elif visualization_type == "bar":
                fig = px.bar(data, x=column, y=data.columns[-1], title=f"{column.capitalize()} vs Target")
